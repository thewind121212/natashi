...
class WebSocketHandler {
    ...
    setupQueueManager() {
        this.queueManager.on('update', (state) => {
            this.broadcastJson({
                type: 'queueUpdated',
                queue: state.queue,
        ...
        try {
            ...
            if (message.action === 'play' && message.url) {
                ...
                try {
                    ...
                    if (metadata.is_playlist) {
                        ...
                        for (const entry of playlist.entries) {
                            this.queueManager.addTrack(entry.url, entry.title, entry.duration, entry.thumbnail);
                        }
                        ...
                        this.queueManager.addTrack(url, metadata.title, metadata.duration, metadata.thumbnail);
                ...
                try {
                    ...
                    this.queueManager.addTrack(message.url, metadata.title, metadata.duration, metadata.thumbnail);
                    this.log('nodejs', `Added to queue: ${metadata.title}`);
